...
def prepare_dom_data(archive, rel_map, dom, start=0, data_kind='tensor', device='cpu'):
    ...
    # print("prepare_dom_data: ", data)
    print("num of lebal 0: ", n_0)
    print("num of lebal 1: ", n_1)
    print("num of lebal 2: ", n_2)

    return get_packed_data(data, data_kind, device)


# 获取个体中的value值，用于训练
def prepare_data(archive):
    data = []
    for ind in archive:
        # data.append(ind.normalized_var)
        data.append(ind.values)
        # data.append(ind.wvalues)
...
def acc_for_class(true_labels, pred_labels, cls):
    ...
    if len(pls) == 0:
        print("No samples for class " + str(cls) + " in the batch")
        return 1
